# Pokédex Interactiva ⚡

Proyecto realizado como práctica de **HTML**, **CSS** y **JavaScript**, utilizando la [PokéAPI](https://pokeapi.co/) para obtener datos en tiempo real sobre los Pokémon.

## ✨ Funcionalidades
- Buscar Pokémon por nombre o número de Pokédex.
- Ver detalles como:
  - Imagen
  - Nombre
  - Tipo(s)
  - ID
  - Estadísticas (HP, Ataque, Defensa, etc.)
- Navegar entre Pokémon con botones **anterior/siguiente**.
- Alternar entre versión normal y **shiny** del Pokémon.

## 🛠️ Tecnologías usadas
- HTML5
- CSS3 (con flexbox y grid para layout)
- JavaScript (fetch API, DOM manipulation)
- GitHub Pages para el despliegue

## 🚀 Demo
Podés probar la Pokédex online en el siguiente link:  
👉 [Pokédex Interactiva](https://santiagolc.github.io/talentotech-pokedex/)

## 📂 Estructura del proyecto
```
├── index.html
├── pokedex.html
├── styles/
│   └── styles.css
├── scripts/
│   ├── script.js
│   └── utils.js
├── img/
└── README.md
```

## 🙌 Créditos
- [PokéAPI](https://pokeapi.co/) por proveer la data de los Pokémon.  
- Proyecto creado como parte del curso **Talento Tech - Frontend con JavaScript**.
